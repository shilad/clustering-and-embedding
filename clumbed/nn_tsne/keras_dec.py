'''
Keras implementation of deep embeddering to improve clustering, inspired by:
...
class DeepEmbeddingClustering(object):
    ...
    #DeepEmbeddingClustering is its own class
    def __init__(self,
                 n_clusters,
                 input_dim,
                 encoded=None,
        ...
        self.encoders_dims = [self.input_dim, 50, 50, 250, 500]
    ...
    def cluster(self, X, y=None,
        ...
        while train:
            ...
            if iteration % save_interval == 0:
                ...
                plt.scatter(z_2d[:, 0], z_2d[:, 1], marker='.',c=[matplotlib.cm.spectral((float(i)+1) / 10) for i in kMeans.labels_]);
                plt.savefig('c'+str(iteration)+'.png')
                plt.clf()

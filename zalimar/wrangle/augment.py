...
def augment_label_svd(input_dir, output_dir, label_dims=20, label_weight=0.2):
    """
    ...
    merged.to_csv(output_dir + '/vectors_label_%d.tsv' % label_dims, sep='\t')

    return merged


def augment_clusters(input_dir, k=10, clust_weight=0.25):
    """
    ...
    df.to_csv(input_dir + '/cluster_%d.tsv' % k, sep='\t')
    ...
    merged.to_csv(input_dir + '/vectors_kmeans_%d.tsv' % k, sep='\t')

    return merged


def augment_everything(input_dir, k=10, label_dims=20, label_weight=0.2, clust_weight=0.25):
    """
    ...
    df_clust = augment_clusters(input_dir, k)
    df_lab = augment_label_svd(input_dir, label_dims)
    ...
    merged.to_csv(input_dir + '/vectors_label_%d_kmeans_%d.tsv' % (label_dims, k), sep='\t')

if __name__ == '__main__':
    augment_everything('./data/simple_500')